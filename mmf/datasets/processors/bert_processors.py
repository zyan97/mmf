...
class MaskedMultiSentenceBertTokenizer(BertTokenizer):
    def __init__(self, config, *args, **kwargs):
        ...
        self.fusion_strategy = config.get("fusion", "concat")
        self._probability = getattr(config, "mask_probability", 0.15)
        self._type = getattr(config, "type", None)
        self._with_sentence_seperator = getattr(
            config, "with_sentence_separator", False
        )
    ...
    def _rand_sentences(self, sentence_tokens):
        ...
        for idx, sentence_token in enumerate(sentence_tokens):
            if sum(len(x) for x in sentence_tokens[idx:]) < length_to_fullfill:
                max_start = idx
                break
            length_to_fullfill -= len(sentence_separater)

        random_idx = random.randint(0, max_start - 1 if max_start > 0 else 0)
